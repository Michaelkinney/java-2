...
public class Order {
    private burger_shop.Burger orderBurger;
    private final ArrayList<burger_shop.Topping> toppingsList;
    private final ArrayList<burger_shop.Side> sidesList;
    private final ArrayList<burger_shop.Burger> burgersList;
    private final ArrayList<burger_shop.Drink> drinksList;

    private static final DecimalFormat priceFormat = new DecimalFormat("##0.00");

    public Order(burger_shop.Burger orderBurger, burger_shop.Bread orderBread, burger_shop.Meat orderMeat) {
    ...
    public Order(burger_shop.Meal orderMeal) {
    ...
    public void addToppings(burger_shop.Topping... addedToppings) {
        for(burger_shop.Topping t: addedToppings){
            ...
            } else if(orderBurger == burger_shop.Burger.Health && !t.isHealthy()) {
    ...
    public void addSides(burger_shop.Side... addedSides) {
        this.sidesList.addAll(Arrays.asList(addedSides));
    }

    public void addBurger(burger_shop.Burger addedBurger) {
        this.burgersList.add(addedBurger);
    }

    public void addDrink(burger_shop.Drink addedDrink) {
        this.drinksList.add(addedDrink);
    }

    public void printOrderTotal() {
        double orderTotal = 0.00;
        for(burger_shop.Burger burger: burgersList) {
            burger.printBasicBurgerPrice();
            orderTotal += burger.basicBurgerPrice();
        }
        for(burger_shop.Topping topping: this.toppingsList) {
        ...
        for(burger_shop.Side side: sidesList) {
        ...
        for(burger_shop.Drink drink: drinksList) {
    ...
    public void purchaseOrder() {
        System.out.println("Thank you for eating at Bill's Burgers!");
    }
}